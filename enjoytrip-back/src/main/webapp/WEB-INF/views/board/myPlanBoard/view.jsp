      ...
      <c:if test="${!empty article.fileInfos}">
        <div class="mt-3">
          <ul>
            ...
            <li>${file.originalFile} <a href="${root}/myPlanFile/download/${file.saveFolder}/${file.originalFile}/${file.saveFile}">[다운로드]</a>
              </c:forEach>
          </ul>
        </div>
      </c:if>
        ...
        <c:if test="${login.userId eq article.userId}">
          ...
          <script>
            document.querySelector("#btn-mv-modify").addEventListener("click", function () {
              ...
              form.setAttribute("action", "${root}/myPlanBoard/modify");
              form.submit();
            });
            ...
            document.querySelector("#btn-delete").addEventListener("click", function () {
              if(confirm("정말 삭제하시겠습니까?")) {
                let form = document.querySelector("#form-no-param");
                form.setAttribute("action", "${root}/myPlanBoard/delete");
                form.submit();
              }
            });
          </script>
